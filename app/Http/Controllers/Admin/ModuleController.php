...
class ModuleController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('modules.configuration.module.index')->with('modules', $modules);
    }

    public function add()
    {
        return view('modules.configuration.module.add');
    }

    public function create()
    {
        $rules = ['module' => 'required | unique:modules'];
        $validator = Validator::make(Input::all(), $rules);

        if ($validator->fails()) return redirect()->back()->withInput()->withErrors($validator);

        $objectModule = new Module();
        $objectModule->module = trim(Input::get('module'));
