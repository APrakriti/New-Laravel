...
class UserTypeController extends Controller
{
	...
	{   
		...
		return view('modules.configuration.usertype.index')->with('user_types', $user_types);
	}	

	public function add()
	{
		return view('modules.configuration.usertype.add');
	}
	...
	{
		try{
			$userType = UserType::where('id', '>', '2')->findOrFail($id);
			return view('modules.configuration.usertype.edit')->with('userType', $userType);
	...
	public function update($id)
	{
		...
		try{
			$objectUserType = UserType::where('id', '>', '2')->findOrFail(Input::get('id'));
			$objectUserType->user_type_name = trim(Input::get('user_type_name'));
			$objectUserType->save();
    ...
    {
        $rules = ['usertype_id'=>'required|exists:user_types,id'];
        $validator = Validator::make($request->only('usertype_id'), $rules);
        if($validator->fails())
            return response()->json(['status'=>'error', 'message'=>$validator->errors()->all()], 422);
        
        $usertype = Tier::find($request->usertype_id);
        
        $usertype->delete();

        

        $message = 'Your usertype is deleted successfully.';
        return response()->json(['status'=>'ok', 'message'=>$message, 'usertype'=>$usertype], 200);
    }
	...
	*/
	public function accesslist($id)
	{
		...
		return view('modules.configuration.usertype.accesslist')->with('usertype', $usertype);
	}
	...
	*/
	...
	public function trashes()
	{
		...
		return view('modules.configuration.usertype.trash')->with('user_types', $user_types);
	}
