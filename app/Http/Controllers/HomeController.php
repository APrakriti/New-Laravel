...
class HomeController extends Controller
{
    ...
    public function getHotelInquiry()
    {
        $countries = Country::all();
        $relatedPackages = Package::where('is_active', 1)
            ...
            ->take(10)
            ->get();
        return view('frontend.hotel_inquiry')
            ->with('countries', $countries)
            ->with('relatedPackages', $relatedPackages);

    }

    public function postHotelInquiry(Request $request)
    {
        $rules = [
            'email_address' => 'required',
            'arrival_date' => 'required',
        ...
        $subject = "Hotel Inquiry Received .";

        $content = '<table cellspacing="0" cellpadding="0" width="100%">';
        $content .= '<tr><td><p>Dear <strong>admin,</strong> <br />';
        $content .= '<br />A visitor <strong>' . $request->first_name . ' ' . $request->last_name . '</strong> has inquiry for the Hotel</td>';
        $content .= '</table>';
        ...
        $request->session()->flash('type', 'success');
        $request->session()->flash('message', 'Thank You <br>Inquiry for <strong>Hotel booking</strong> has been received. we will be in contact with you shortly.<br>
                '.env('SITE_NAME').'');
        return view('frontend.success');
